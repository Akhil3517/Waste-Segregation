...
const ReportGarbage = ({ navigation }) => {
  ...
  const requestLocationPermission = async () => {
    try {
      ...
      // Request permission directly - this will show the native Android dialog
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.CAMERA,
        {
          ...
          message: 'This app needs access to your camera to take photos for garbage reports.',
          buttonNeutral: 'Ask Me Later',
          buttonNegative: 'Cancel',
          buttonPositive: 'OK',
        }
  ...
  const getCurrentLocation = async () => {
    try {
      // Check permission first - same pattern as camera
      if (!hasLocationPermission) {
        // Request permission directly - let the phone show the native dialog
